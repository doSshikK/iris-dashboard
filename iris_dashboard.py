...
# ------------- Страница: Метрики и выводы -------------
    ...
    with col1:
        ...
        - **Setosa** отделяется очень чётко от остальных видов
        - **Versicolor и virginica** имеют частичное перекрытие из-за биологического сходства
        - **Признаки лепестка** наиболее информативны для классификации
        """)
    with col2:
        corr = df_filtered.iloc[:, :4].corr()
        st.markdown("**🔥 Сильные корреляции:**")
        st.markdown(f"""
        - Длина и ширина лепестка: **{corr.loc['petal length (cm)', 'petal width (cm)']:.2f}**
        - Длина лепестка и чашелистика: **{corr.loc['petal length (cm)', 'sepal length (cm)']:.2f}**
        - Ширина лепестка и чашелистика: **{corr.loc['petal width (cm)', 'sepal length (cm)']:.2f}**
        """)
    ...
    with col_clust1:
        st.metric("**Оптимальное количество кластеров**", "3", 
                 help="Определено методом локтя и подтверждено дендрограммой")
    with col_clust2:
        st.metric("**Качество кластеризации (Silhouette)**", f"{sil3:.3f}",
                 delta="Удовлетворительное" if sil3 > 0.4 else "Низкое",
                 help="Silhouette Score от -1 до 1, чем выше - тем лучше")
    
    st.info("""
    **Выводы по кластеризации:**
    - K-means с k=3 соответствует трём биологическим видам ирисов
    - Качество кластеризации удовлетворительное (Silhouette Score > 0.4)
    - Вид setosa чётко отделяется в отдельный кластер
    """)
    ...
    if df_filtered['species'].nunique() >= 2:
        ...
        importance_df = pd.DataFrame({
            ...
            'LR Важность': importance_lr,
            'RF Важность': importance_rf
        })
        
        # Сравнение точности моделей
        col_acc1, col_acc2, col_acc3 = st.columns(3)
        with col_acc1:
            st.metric("**Logistic Regression**", f"{acc_lr_full:.1%}",
                     delta="Высокая точность" if acc_lr_full > 0.9 else "Средняя точность")
        with col_acc2:
            st.metric("**Random Forest**", f"{acc_rf_full:.1%}",
                     delta="Высокая точность" if acc_rf_full > 0.9 else "Средняя точность")
        with col_acc3:
            diff = abs(acc_lr_full - acc_rf_full)
            better_model = "LR" if acc_lr_full > acc_rf_full else "RF" if acc_rf_full > acc_lr_full else "Одинаково"
            st.metric("**Разница**", f"{diff:.1%}",
                     delta=f"Лучше {better_model}" if better_model != "Одинаково" else "Равны")
        
        # Сравнение важности признаков
        st.subheader("📊 Сравнение важности признаков")
        
        fig_compare, ax_compare = plt.subplots(figsize=(10, 5))
        x = np.arange(len(X_all.columns))
        width = 0.35
        
        ax_compare.bar(x - width/2, importance_df['LR Важность'], 
                      width, label='Logistic Regression', color='lightgreen', alpha=0.8)
        ax_compare.bar(x + width/2, importance_df['RF Важность'], 
                      width, label='Random Forest', color='lightblue', alpha=0.8)
        
        ax_compare.set_xlabel('Признаки')
        ax_compare.set_ylabel('Важность')
        ax_compare.set_title('Сравнение важности признаков в разных моделях')
        ax_compare.set_xticks(x)
        ax_compare.set_xticklabels(X_all.columns, rotation=45)
        ax_compare.legend()
        st.pyplot(fig_compare)
        
        # Таблица с важностью признаков
        st.dataframe(importance_df.sort_values('LR Важность', ascending=False).round(4), 
        ...
        error_analysis = pd.DataFrame({
            ...
            'Общая точность': [acc_lr_full, acc_rf_full],
            'Ошибок setosa': [cm_lr[0].sum() - cm_lr[0, 0], cm_rf[0].sum() - cm_rf[0, 0]],
            'Ошибок versicolor': [cm_lr[1].sum() - cm_lr[1, 1], cm_rf[1].sum() - cm_rf[1, 1]],
            'Ошибок virginica': [cm_lr[2].sum() - cm_lr[2, 2], cm_rf[2].sum() - cm_rf[2, 2]]
        })
        
        st.dataframe(error_analysis.round(3), use_container_width=True)
        
        st.info("""
        **Наблюдения по ошибкам:**
        - Обе модели идеально классифицируют вид **setosa**
        - Основные ошибки происходят между видами **versicolor** и **virginica**
        - Random Forest показывает стабильно высокую точность
        """)

    st.markdown("---")
    
    # ------ Рекомендации и выводы ------
    st.subheader("💡 Рекомендации и практические выводы")
    
    col_rec1, col_rec2 = st.columns(2)
    
    with col_rec1:
        st.markdown("""
        ### 🎯 Для классификации ирисов:
        **Рекомендуемые модели:**
        1. **Random Forest** - высокая точность и интерпретируемость
        2. **Logistic Regression** - быстрая и эффективная для линейно разделимых данных
        
        **Ключевые признаки:**
        - Длина лепестка (petal length) - самый важный
        - Ширина лепестка (petal width) - второй по важности
        - Признаки чашелистика имеют меньшую значимость
    ...
    with col_rec2:
        ...
        **Улучшение моделей:**
        1. Добавить кросс-валидацию для более надёжной оценки
        2. Попробовать другие алгоритмы (SVM, Gradient Boosting)
        3. Использовать ансамбли моделей
        
        **Развитие дашборда:**
        1. Добавить возможность загрузки пользовательских данных
        2. Реализовать автоматический подбор гиперпараметров
        3. Добавить экспорт результатов в различные форматы
        
        **Для production:**
        1. Добавить мониторинг дрейфа данных
        2. Реализовать A/B тестирование моделей
        3. Добавить логирование предсказаний
        """)
